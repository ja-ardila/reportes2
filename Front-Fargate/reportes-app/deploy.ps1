...
function Build-AndPushImage {
    ...
    try {
        ...
        Write-Host "🔨 Construyendo aplicación Angular..." -ForegroundColor Yellow
        npm run build
        if ($LASTEXITCODE -ne 0) { throw "Error construyendo aplicación" }
        
        # Verificar que se creó la carpeta dist
        if (-not (Test-Path "dist")) {
            throw "No se creó la carpeta dist después de la construcción"
        }
        ...
        echo $loginCommand | docker login --username AWS --password-stdin $EcrRepositoryUri
